...
> **참고:** 1.2(트랜잭션 분리), 1.4(캐싱), 2.8(병렬화) 항목은 검토 후 제거/완료 처리됨
...
### ~~1.2 주문 실행 중 장시간 트랜잭션~~ ❌ 제거됨

> **제거 사유:** 주문 실행 사이클(Order 생성 → Upbit 호출 → 포지션 업데이트)은 원자적 단위로 유지되어야 데이터 일관성이 보장됨. 트랜잭션 분리 시 중간 실패로 불완전한 주문 상태가 발생할 수 있음.

---

### 1.3 ~~HTTP 클라이언트 종료 누락~~ → 권장으로 이동

> **이동 사유:** 장시간 운영에서 실질적 문제 발생 가능성 낮음. 모범 사례 수준의 개선 사항으로 "권장" 섹션 3.8로 이동.

---

### ~~1.4 Balance/Ticker 캐싱 부재~~ ❌ 제거됨

> **제거 사유:**
> - **Ticker**: 신호 생성 시점의 정확한 실시간 가격이 필요하므로 캐싱하면 안 됨
> - **Balance**: 현재 구조에서 중복 호출 없음 (신호 생성 시 1회, 주문 검증 시 1회)
...
### 3.8 HTTP 클라이언트 종료 (긴급에서 이동)

**파일들:**
- `backend/src/clients/upbit/public_api.py`
- `backend/src/clients/upbit/private_api.py`
- `backend/src/clients/slack_client.py`

**문제:** 싱글톤 클라이언트의 `.close()` 메서드가 앱 종료 시 호출되지 않음

**해결:** `main.py` lifespan에서 클라이언트 종료 추가
...
**영향:** Graceful shutdown 개선, 경고 로그 제거
...
| 2 | 설정값 캐싱 | DB 쿼리 99%↓ | 낮음 | 1시간 |
| 3 | DB 인덱스 추가 | 쿼리 20-50%↑ | 낮음 | 30분 |
| 4 | 에러 복구 전략 | 안정성 향상 | 중간 | 2시간 |
| 5 | 트랜잭션 격리 레벨 | 데이터 일관성 | 낮음 | 10분 |
| 6 | 함수 분리 리팩토링 | 가독성 향상 | 중간 | 2시간 |
| 7 | HTTP 클라이언트 종료 | Graceful shutdown | 낮음 | 30분 |

> **제거된 항목:** Balance/Ticker 캐싱, 트랜잭션 분리, 비동기 병렬화
...
- [ ] `database.py`: 트랜잭션 격리 레벨 설정

### Phase 2 (1주일)
- [ ] `utils/cache.py`: TTLCache 구현 (설정값용)
- [ ] 설정값 캐싱 적용
- [ ] `main.py`: lifespan에 클라이언트 종료 추가
...
1. **가장 시급:** DB 커넥션 풀 + 설정값 캐싱 (DB 쿼리 대폭 감소)
2. **안정성:** 에러 복구 전략 + 트랜잭션 격리 레벨
