...
export function getPortfolioSummaryMock(): PortfolioSummary {
  ...
  return {
    ...
    total_realized_pnl: Math.round(currentValue - totalDeposit),
    net_realized_pnl: Math.round(currentValue - totalDeposit), // 수수료 0 가정
    today_return_pct: todayReturnPct,
    today_realized_pnl: Math.round(todayRealizedPnl),
    total_trades: 47,
    win_count: 28,
    loss_count: 19, // win_rate 59.57% = 28/(28+19)와 정합
    win_rate: 59.57,
