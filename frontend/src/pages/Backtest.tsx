      ...
      {result.status === "COMPLETED" && (
        <div className="grid grid-cols-2 gap-3 mt-3">
          <div>
            ...
            <p className={`text-sm font-bold courier ${
              Number(result.total_return_pct) >= 0 ? "text-emerald-400" : "text-rose-400"
            }`}>
        ...
        <>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            <div className="p-4 bg-white/5 rounded-xl border border-white/10">
              ...
              <p className={`text-xl font-bold courier ${
                Number(result.total_return_pct) >= 0 ? "text-emerald-400" : "text-rose-400"
              }`}>
                {formatPercent(result.total_return_pct)}
              </p>
                  ...
                  <tbody>
                    {result.trade_history.slice(0, 20).map((trade: TradeRecord, index: number) => (
                      <tr key={index} className="border-b border-white/5">
                        ...
                        <td className={`py-2 text-right courier ${
