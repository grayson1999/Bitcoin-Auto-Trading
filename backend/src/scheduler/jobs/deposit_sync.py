...
Upbit 입금 원장(/v1/deposits)을 BalanceAdjustment로 정기 동기화하여
포트폴리오 total_deposit(원금)이 실제 입금을 정확히 반영하도록 한다.
...
async def sync_deposits_job() -> None:
    ...
    Upbit 입금 원장을 조회해 봇 시작일 이후 신규 입금을 BalanceAdjustment에
    기록한다. 중복은 date+amount로 방지.
    """
    async with track_job("deposit_sync"), async_session_factory() as session:
        try:
            portfolio_service = await get_portfolio_service(session)
            recorded = await portfolio_service.sync_deposits_from_upbit()
            await session.commit()

            if recorded > 0:
                logger.info(f"입금 동기화: {recorded}건 신규 기록")

        except Exception as e:
            await session.rollback()
            logger.exception(f"입금 동기화 작업 오류: {e}")
            raise
