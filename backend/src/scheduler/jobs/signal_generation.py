    ...
    async with track_job("signal_generation"), async_session_factory() as session:
        try:
            ...
            if signal.signal_type == SignalType.BUY.value:
                # 1. 최소 신뢰도 필터: BUY 신뢰도 < 0.5 → HOLD 변환
                if signal.confidence < 0.5:
                    logger.info(
                        f"BUY 신뢰도 필터: {signal.confidence:.2f} < 0.50 → HOLD 변환"
                    )
                    signal.signal_type = SignalType.HOLD.value
                    signal.reasoning = (
                        signal.reasoning or ""
                    ) + f" [저신뢰도 BUY 필터: {signal.confidence:.2f} < 0.50]"

                # 2. 연속 실패 필터: 최근 48시간 BUY 5개 중 4개 이상 실패 → 차단
                elif await _check_recent_buy_failure(session):
                    logger.info(
                        "BUY 연속 실패 필터: 최근 48시간 BUY 정확도 < 20% → HOLD 변환"
                    )
                    signal.signal_type = SignalType.HOLD.value
                    signal.reasoning = (
                        signal.reasoning or ""
                    ) + " [연속 실패 필터: 최근 BUY 정확도 < 20%]"
...
async def _check_recent_buy_failure(session) -> bool:
    """
    최근 48시간 내 평가된 BUY 신호 중 실패율 확인

    평가된 BUY 5개 이상 중 4개 이상 실패(정확도 < 20%)이면 True 반환.

    Returns:
        bool: 연속 실패로 BUY 차단 필요 여부
    """
    ...
    if accuracy < 0.2:
        logger.warning(
            f"BUY 연속 실패 감지: 최근 48시간 {total}건 중 {correct}건 성공 "
            f"(정확도: {accuracy:.1%})"
        )
        return True
