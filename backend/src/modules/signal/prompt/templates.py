"""
코인 유형별 AI 프롬프트 템플릿 모듈 v2.3 (BUY 정확도 개선)

이 모듈은 코인 유형(메이저/밈코인/알트코인)에 따라 최적화된 AI 프롬프트를 제공합니다.

v2.3 주요 변경:
- BUY 임계값 강화: score >= 0.3 → BUY (기존 0.2)
- BEARISH 레짐 매수 조건 강화: score >= 0.4 (RSI < 25 + 거래량 급증 + 모멘텀 전환)
- 포지션 보유 시 추가 매수 자제 규칙 추가
- SELL, HOLD 기준 유지
"""
...
# === 메이저 코인용 시스템 프롬프트 v2.3 (BUY 정확도 개선) ===
...
- **BEARISH**: 매수 극도로 신중. RSI < 25 반등 + 거래량 급증 + 모멘텀 전환 확인 시에만 (score >= 0.4). 단순 반등 기대로 매수하지 마세요
...
**신호 결정**:
- score >= 0.3 → BUY (BEARISH에서는 score >= 0.4)
- score <= -0.2 → SELL
- -0.2 < score < 0.3 → HOLD
...
- RSI 과매도(< 25) + 거래량 급증 + 모멘텀 전환 확인 → 반등 매수 기회 (score 0.4~0.6)
...
- 급락 후 RSI < 25 + 거래량 폭발 + 모멘텀 전환 → 반등 매수 기회 (score 0.4~0.6)
...
# === 알트코인용 시스템 프롬프트 v2.3 (BUY 정확도 개선) ===
