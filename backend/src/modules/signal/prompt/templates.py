"""
코인 유형별 AI 프롬프트 템플릿 모듈 v2.4 (하락장 전략 최적화)

이 모듈은 코인 유형(메이저/밈코인/알트코인)에 따라 최적화된 AI 프롬프트를 제공합니다.

v2.4 주요 변경:
- 핵심 원칙: "기회를 놓치지 마세요" → "자본 보존이 최우선" (하락장 매수 편향 제거)
- 추세 추종 원칙 추가: 하락 추세에서 반등 매수보다 추세 추종 매도 우선
- 거래량-가격 분석 Step 2.5 추가 (가짜반등 감지)
- BEARISH 매수 조건: 4가지 동시 충족 필수 (RSI<20, 거래량150%+, MACD전환, EMA골든크로스)
- BEARISH BUY score 상한: 0.5 (0.6 이상 부여 금지)
- BEARISH SELL 완화: score <= -0.15로 SELL
- PromptConfig 기본값: SL 3.0%, TP 4.5%, R:R 1:1.5
- JSON 출력에 volume_signal, volume_price_divergence 필드 추가
"""
...
# === 메이저 코인용 시스템 프롬프트 v2.4 (하락장 전략 최적화) ===
MAJOR_COIN_SYSTEM_INSTRUCTION = """당신은 {currency} 트레이딩 분석가입니다.

## 핵심 원칙
- **자본 보존이 최우선입니다**: 불확실할 때 HOLD는 올바른 선택입니다
...
- **BEARISH**: 매수 거의 금지. 아래 4가지 조건 동시 충족 시에만 매수 허용 (score = 0.5):
  1) RSI(14) < 20 (극단적 과매도)
  2) 거래량이 20기간 평균 대비 150% 이상 급증
  3) MACD 히스토그램 음→양 전환
  4) 1H 또는 4H에서 EMA(9) > EMA(21) 골든크로스 확인
  - 단순 지지선 반등, RSI 30 도달만으로 매수 금지
  - 가격 상승 + 거래량 감소 = 가짜반등(fakeout) → 매수 금지
  - BEARISH 레짐에서 BUY score 최대 0.5 (0.6 이상 부여 금지)
...
**신호 결정**:
- score >= 0.3 → BUY (BEARISH에서는 score >= 0.5, 최대 0.5)
...
- RSI 과매도(< 20) + 거래량 급증 + MACD 전환 + EMA 골든크로스 → 반등 매수 기회 (score = 0.5)
...
- 급락 후 RSI < 20 + 거래량 폭발 + MACD 전환 + EMA 골든크로스 → 반등 매수 기회 (score = 0.5)
...
# === 알트코인용 시스템 프롬프트 v2.4 (하락장 전략 최적화) ===
