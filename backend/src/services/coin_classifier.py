...
def is_major_coin(currency: str) -> bool:
    """
    메이저 코인 여부 확인
    ...
    Returns:
        bool: 메이저 코인이면 True
    """
    return get_coin_type(currency) == CoinType.MAJOR


def is_memecoin(currency: str) -> bool:
    """
    밈코인 여부 확인
    ...
    Returns:
        bool: 밈코인이면 True
    """
    return get_coin_type(currency) == CoinType.MEMECOIN


def get_coin_type_description(coin_type: CoinType) -> str:
    """
    코인 유형 설명 반환
    ...
    """
    descriptions = {
        CoinType.MAJOR: "시가총액 상위 메이저 코인 - 안정적, 기술적 분석 중시",
        CoinType.MEMECOIN: "밈코인 - 고변동성, 모멘텀/거래량 중시, 빠른 대응 필요",
        CoinType.ALTCOIN: "일반 알트코인 - 중간 수준 변동성, 균형잡힌 전략",
    }
    return descriptions.get(coin_type, "알 수 없는 코인 유형")
