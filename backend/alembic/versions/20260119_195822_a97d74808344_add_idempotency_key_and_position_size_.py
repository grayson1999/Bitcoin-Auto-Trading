...
def upgrade() -> None:
    ...
    op.alter_column('market_data', 'low_price',
               ...
               existing_nullable=False)
    op.alter_column('market_data', 'trade_count',
    ...
    op.add_column('orders', sa.Column('idempotency_key', sa.String(length=64), nullable=True, comment='재시도 시 중복 방지용 키'))
    ...
    op.alter_column('risk_events', 'order_id',
    ...
    op.create_index(op.f('ix_risk_events_order_id'), 'risk_events', ['order_id'], unique=False)
    ...
    op.alter_column('trading_signals', 'market_data_id',
               existing_type=sa.BIGINT(),
    ...
    op.alter_column('trading_signals', 'signal_type',
               ...
               existing_nullable=False)
    op.alter_column('trading_signals', 'confidence',
               existing_type=sa.NUMERIC(precision=3, scale=2),
               comment='신뢰도 점수 (0~1)',
               existing_comment='Confidence score (0-1)',
               existing_nullable=False)
    op.alter_column('trading_signals', 'reasoning',
               existing_type=sa.TEXT(),
               comment='AI 분석 근거',
               existing_comment='AI analysis reasoning',
               existing_nullable=False)
    op.alter_column('trading_signals', 'created_at',
               existing_type=postgresql.TIMESTAMP(timezone=True),
               comment='신호 생성 시간',
               existing_comment='Signal creation time',
               existing_nullable=False)
    op.alter_column('trading_signals', 'model_name',
               existing_type=sa.VARCHAR(length=50),
               comment='사용된 AI 모델명',
               existing_comment='AI model name used',
               existing_nullable=False)
    op.alter_column('trading_signals', 'input_tokens',
               existing_type=sa.INTEGER(),
               comment='입력 토큰 수',
               existing_comment='Input tokens count',
               existing_nullable=False)
    op.alter_column('trading_signals', 'output_tokens',
               existing_type=sa.INTEGER(),
               comment='출력 토큰 수',
    ...
    op.create_index(op.f('ix_trading_signals_market_data_id'), 'trading_signals', ['market_data_id'], unique=False)
...
def downgrade() -> None:
    ...
    # ### commands auto generated by Alembic - please adjust! ###
    op.drop_index(op.f('ix_trading_signals_market_data_id'), table_name='trading_signals')
    op.drop_index(op.f('ix_trading_signals_created_at'), table_name='trading_signals')
    op.create_index(op.f('idx_signal_market_data_id'), 'trading_signals', ['market_data_id'], unique=False)
    op.create_index(op.f('idx_signal_created_at'), 'trading_signals', ['created_at'], unique=False)
    op.alter_column('trading_signals', 'output_tokens',
               existing_type=sa.INTEGER(),
    ...
    op.create_unique_constraint(op.f('system_configs_key_key'), 'system_configs', ['key'], postgresql_nulls_not_distinct=False)
    op.create_index(op.f('idx_system_config_key'), 'system_configs', ['key'], unique=True)
    ...
    op.create_index(op.f('idx_risk_event_order_id'), 'risk_events', ['order_id'], unique=False)
    op.create_index(op.f('idx_risk_event_created_at'), 'risk_events', ['created_at'], unique=False)
    op.alter_column('risk_events', 'order_id',
               existing_type=sa.BIGINT(),
    ...
    op.create_unique_constraint(op.f('positions_symbol_key'), 'positions', ['symbol'], postgresql_nulls_not_distinct=False)
    ...
    op.create_index(op.f('idx_market_data_symbol'), 'market_data', ['symbol'], unique=False)
    op.alter_column('market_data', 'trade_count',
               existing_type=sa.INTEGER(),
    ...
    op.create_unique_constraint(op.f('daily_stats_date_key'), 'daily_stats', ['date'], postgresql_nulls_not_distinct=False)
